...
export function AddExpenseBar({
  ...
  return (
    <div className="rounded-2xl bg-card p-3 shadow-sm shadow-black/5 ring-1 ring-card-border">
      {(error || summary) && (
        <p className={`mb-1.5 px-1 text-xs ${error ? "text-danger" : "text-ink-muted"}`}>
          {error || summary}
        </p>
      )}
